...
import com.novacloud.data.formula.Category;
 ...
 *
 * @author jindw
 * see {@literal org.mozilla.javascript.NativeGlobal }
 */
public abstract class ECMA262Impl {
	public static final Logger logger = LoggerFactory.getLogger(ECMA262Impl.class);
...
//		setupGlobal(calculater, Math.class,Category.Math);
		setupGlobal(calculater, DateFunctions.class,Category.Date);
		setupGlobal(calculater, StringFunctions.class,Category.String);
		setupGlobal(calculater, HtmlFunctions.class,Category.Html);
//		setupGlobal(calculater, ArrayFunctions.class,Category.Array);
		setupGlobal(calculater, LogicalFunctions.class,Category.Logical);
		setupGlobal(calculater, TypeConversionFunctions.class,Category.TypeConversion);


	}
	private static void setupGlobal(ExpressionFactoryImpl calculater,
				Class  impl,Category category) {
		try {
			for (Method method : impl.getMethods()) {
				 if(method.getDeclaringClass()==impl && Modifier.isStatic(method.getModifiers())){
					 //note: if have same name method, later will override previous.
					 calculater.addGlobalFn(method.getName().toUpperCase(), method,category);
				 }
			}
		}   catch (Exception e){
			logger.warn("setup global function error:{}", impl.getName(),e);
		}
	}
			...
			for (Method method : dms) {
				if (method.getDeclaringClass() == impl) {
					...
					for (Class<?> type : forClass) {
						calculater.addMethod(type, method.getName().toUpperCase(), inv);
					}
				}
			}
		} catch (Exception e) {
