 ...
 * String.prototype.regexReplace(searchValue, replaceValue) 15.5.4.13
 ...
 * String.prototype.regexMatch(regexp) 15.5.4.12 String.prototype.search(regexp)
 */
public class JSString extends JSObject implements Invocable {
	...
	// 15.5.4.11 String.prototype.regexReplace(searchValue, replaceValue)
	public String replace(String thiz, Object[] args) {
		if(args.length>0){
			Object regexp = args[0];
			String replaceValue = getStringArg(args, 1, "undefined");
...
//			System.out.println(replaceValue);
	...
	public String slice(String thiz, Object[] args) {
		...
		int begin = JSArray.toSliceRange(getIntArg(args, 0, 0),
				size);
		int end = JSArray.toSliceRange(getIntArg(args, 1, size),
				size);
		if (begin < end) {
	...
	public Object[] split(String thiz, Object[] args) {
		Object separator = getArg(args, 0, null);
		if (separator == null) {
			return new String[] { thiz };
		}
		final int limit = getIntArg(args, 1, -1);
		JSRegExp exp = JSRegExp.getRegExp(separator);
		Pattern pattern;
		if(exp!=null){
			pattern = exp.pattern;
			return pattern.split(thiz, limit);
		}else{
	...
	public String substring(String thiz, Object[] args) {
		int size = thiz.length();
		int begin = toSubstringRange(getIntArg(args, 0, 0), size);
		int end = toSubstringRange(getIntArg(args, 1, size), size);
	...
	public String substr(String thiz, Object[] args) {
		int size = thiz.length();
		int begin = toSubstringRange(getIntArg(args, 0, 0), size);
		int len = getIntArg(args, 1, size - begin);
		if (len > 0) {
