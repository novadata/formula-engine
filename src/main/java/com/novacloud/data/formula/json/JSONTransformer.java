...
class JSONTransformer {
    ...
    private final Map<Type, JSONDecoder.TypeTransformer<?>> objectFactory = new HashMap<>();
    static {
    ...
    private Object createObject(Map map, Class defaultClass) {
        ...
        if (defaultClass.isMemberClass() && 0 == (defaultClass.getModifiers() & Modifier.STATIC)) {
    ...
    public JSONDecoder.TypeTransformer<?> addFactory(
            JSONDecoder.TypeTransformer<?> factory) {
        Type type = ReflectUtil.getParameterizedType(factory.getClass(),
                JSONDecoder.TypeTransformer.class, 0);
        return this.objectFactory.put(type, factory);
    }

    private Object require(Object value, Type type) throws ClassNotFoundException, IllegalAccessException, InstantiationException, InvocationTargetException {
        ...
        if (type instanceof Class) {
            ...
            if (result.equals(SIMPLE_NOT_FOUND)) {
                return result;
            }
        } else {
        ...
        if (value instanceof List) {// 折腾，就是为了少调用一次 Class.forName
            ...
            JSONDecoder.TypeTransformer<?> of = objectFactory.get(type);
            if (of != null) {
    ...
    //todo: fixme, param type not used
