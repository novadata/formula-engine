...
import com.novacloud.data.formula.json.JSONTokenizer;

import java.util.*;

import static com.novacloud.data.formula.ExpressionToken.*;
import static com.novacloud.data.formula.impl.TokenImpl.BRACKET_BEGIN;
import static com.novacloud.data.formula.impl.TokenImpl.BRACKET_END;
/**
...
public class ExpressionParser extends JSONTokenizer {
	...
	protected final ArrayList<TokenImpl> tokens = new ArrayList<>();
...
//	this.aliasMap = aliasMap;
	public TokenImpl parseEL() {
		...
		prepareSelect();
		LinkedList<TokenImpl> stack = new LinkedList<>();
		try {
	...
	private List<TokenImpl> right(List<TokenImpl> tokens) {
		LinkedList<List<TokenImpl>> rightStack = new LinkedList<>();
		rightStack.addFirst(new ArrayList<TokenImpl>()); // 存储右序表达式

		LinkedList<TokenImpl> buffer = new LinkedList<>();
